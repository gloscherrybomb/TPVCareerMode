...
function generateSeasonImplications(raceData, seasonData, raceContext) {
  const { position, gcPosition, gcGap } = raceData;
  const {
    stagesCompleted,
    nextEventName,
    nextEventNumber,
  ...
  if (stageInfo) {
    if (stageInfo.stageNumber === 1) {
      ...
      // Stage 3 complete - tour finished
      if (gcPosition === 1) {
        implications = `Local Tour champion. Three stages, countless attacks defended, and you held the leader's jersey to the finish. This is what multi-day racing demands: consistency, strength, and the ability to suffer efficiently day after day. The overall classification is yours.`;
      } else if (gcPosition <= 3) {
        implications = `The Local Tour is complete: ${gcPosition}${getOrdinal(gcPosition)} overall, ${formatGapText(gcGap)} behind the winner. A podium finish in a three-day stage race is an achievement—you proved you can handle the demands of multi-day racing, even if the top step eluded you.`;
      } else {
        implications = `The Local Tour is finished. ${gcPosition}${getOrdinal(gcPosition)} overall isn't where you hoped to be, but you completed all three stages and learned what multi-day racing demands. The experience matters more than the result.`;
      }
      return implications;
    }
    return implications;
