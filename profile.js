...
function displayCareerSummary(userData, stats) {
    ...
    const season = 1; // For now, always season 1
    ...
    for (let i = 1; i <= 15; i++) {
        ...
        if (eventResults && eventResults.position < careerBestPosition) {
            ...
            // Map event number to name (would be better to import from event data)
            const eventNames = {
    ...
    let recentForm = 'consistent';
    if (stats.recentResults && stats.recentResults.length >= 3) {
        const recent = stats.recentResults.slice(0, 3).map(r => r.position);
        const firstPos = recent[0];
        const lastPos = recent[recent.length - 1];
        
        // Check for hot streak (all top 5)
        if (recent.every(p => p <= 5)) {
            recentForm = 'hot';
        }
        // Check for improving (getting better)
        else if (firstPos < lastPos && firstPos <= 10) {
            recentForm = 'improving';
        }
        // Check for struggling (getting worse)
        else if (firstPos > lastPos && firstPos > 15) {
            recentForm = 'struggling';
        }
        // Check for mixed results
        else if (Math.max(...recent) - Math.min(...recent) > 20) {
            recentForm = 'mixed';
        }
    }
    
    // Determine signature achievement
    let signatureAchievement = null;
    const totalAwardsCount = Object.values(stats.awards || {}).reduce((sum, val) => sum + val, 0);
    
    if (stats.totalWins >= 3) {
        signatureAchievement = 'multiple_wins';
    } else if (stats.awards.domination > 0) {
        signatureAchievement = 'dominant_win';
    } else if (stats.awards.zeroToHero > 0 || stats.awards.darkHorse > 0) {
        signatureAchievement = 'comeback';
    } else if (totalAwardsCount >= 8) {
        signatureAchievement = 'consistency';
    } else if (careerBestPosition === 1 || stats.totalPodiums >= 1) {
        signatureAchievement = 'breakthrough';
    }
    
    // Build career data object for generator
    const careerData = {
        totalSeasons: 1, // Will be dynamic in future
