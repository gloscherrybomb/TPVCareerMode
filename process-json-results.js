    ...
    if (fs.existsSync(requestsFilePath)) {
      ...
      for (const entry of entries) {
        ...
        if (botProfileDoc.exists) {
          ...
          console.log(`   Removing completed request for ${botName} (${botUid}) from file`);
          removedFromFile++;
        } else {
          // Keep this entry
          remainingEntries.push(entry);
...
async function processUserResult(uid, eventInfo, results, raceTimestamp, jsonMetadata = null) {
  ...
  // Stage progression and season stats (only for non-special events)
  if (!isSpecialEventResult) {
    ...
    updateData.season1TotalEvents = seasonStats.totalEventsParticipated; // All events including DNF
    updateData.season1Events = seasonStats.totalRaces; // Only finished races (for rate calculations)
    ...
    updateData.season1PodiumRate = seasonStats.podiumRate;
